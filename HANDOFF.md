...
- **結論: 支持されない（確信度: 中）**。当初根拠3件中2件は誤引用（存在しない統計・存在しない分析内容の誤帰属）と判明
  - LoveType16→ラブキャラ64: 記事に「比較シェア構造で拡散」という分析なし。しかもLoveType16自体は単独診断で、相性型の例証として不適切だった
  - Co-Star「米18-25歳女性の25%利用」: 引用元ページにこの統計は存在しない。実際の統計（別サイトJPLoft）は「総DL数の25%が米18-25歳女性由来」で意味が異なる
  - hottolink「診断1億件分析・比較したくなる構造」: このページにその記述なし。実体は別会社（ギミックプラス）の分析で、拡散5要因（自己言語化/ネガのエンタメ化/1枚画像設計/会話のネタ/直感回答）に「比較・相性」は含まれない
- **反例**: 日本語圏で単独診断のバズ実例は複数実在（明日カノ診断=100万回超、恋するへびべあ診断、16Personalities、fasme恋愛動物診断=LINE130万人）。「単独診断はバズらない」は誤り
- **含意**: バズの主変数は「単独か相性か」ではなく、シェアカードの言語・ビジュアル品質。論点2/3（シェアカード品質・デザイン・コピー）の方針が実データと整合。相性機能は招待リンク経由の流入ループ（リテンション装置）として位置づけるのが妥当
- 現状: `compatibility.py`/`generate_compatibility.py`は既にあるが、拡散・シェア専用の導線にはなっていない。アナリティクス計測コードはリポジトリ全体でゼロ（自社実績での比較は不可）
...
**論点3: デザイン・コピーの品質要件（Yuma指示、2026-07-15追加）**
- シェアカード（論点2）は「洗練されたデザイン」「刺さる洗練された文章」が必須要件。単に事実性を落とした雑な短文では不可
